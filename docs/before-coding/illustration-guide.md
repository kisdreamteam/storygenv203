...
* One short **illustration scene** per story page (12 scenes per story), stored in `illustration_prompt`
* Scenes displayed in the UI with Show/Hide toggle; teachers edit the short visual moment only
...
This section defines the official V1 illustration continuity system. Every **copy-assembled production illustration prompt** must include the **locked visual suffix** below. This suffix is not stored in the database and is not shown in the UI — it is injected at copy time. This is the official V1 illustration framing standard.

## Required global suffix

Append this suffix to every copy-assembled production prompt (after scene, character, setting, and mood details):
...
Every **copy-assembled production prompt** that includes a character must use descriptors from resolved `character_profiles` (Supabase when available, factory fallback) aligned with [character-bible.md](character-bible.md) section 14. The LLM `illustration_scene` does not include appearance details.
